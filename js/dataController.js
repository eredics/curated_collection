...
const DataController = (function() {
            ...
            Papa.parse(csvData, {
                ...
                complete: function(results) {
                    ...
                    const artworks = results.data.map((row, index) => {
                        ...
                        const filenameFromCSV = (row.Filename || row.filename || '').toString().trim();
                        ...
                        return {
                            id: row.ID || row.id || `art-${index}`,
                            title: row.Title || row.title || 'Untitled',
                            artist: row.Artist || row.artist || 'Unknown Artist',
                            technique: row.Technique || '',
                            price: row.Price || '', // Ensure price is read correctly
                            size: row.Size || '',
                            // ... copy other relevant properties from row ...
    ...
    return {
        ...
        loadData: function() {
            ...
            updateProgress(10); // <<< Add call here
            ...
            return `
                <article class="artwork" data-id="${artwork.id || ''}">
                    <div class="artwork-image-container">
                        <img
                            src="${imagePath}" // Use the potentially encoded path
                            alt="${artwork.title || ''}"
                            class="artwork-image"
...
// At the end of your DataController module, add:
